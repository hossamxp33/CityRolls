package com.example.osamaomar.shopgate.presentationn.screens.feature.home.favorite.adapter;
...
import com.example.osamaomar.shopgate.presentationn.screens.feature.home.favorite.FavoritesViewModel;
import com.example.osamaomar.shopgate.presentationn.screens.feature.home.productdetailsfragment.ProductDetailsFragment;
import com.example.osamaomar.shopgate.presentationn.screens.feature.home.productfragment.ProductsViewModel;
...
public class AllFavProductsAdapter extends RecyclerView.Adapter<AllFavProductsAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        ...
        try {
            if (productsbysubcats.get(position).getProduct().getProductphotos().size() > 0)
                Glide.with(context.getApplicationContext())
                        .load(productsbysubcats.get(position).getProduct().getProductphotos().get(0).getPhoto()).placeholder(R.drawable.product).dontAnimate()
                        .into(holder.Image);
        ...
        holder.favorite.setOnClickListener(v -> {
            viewModel.DeleteFav(productsbysubcats.get(position).getId());
        });


        viewModel.deleteToFavMutableLiveData.observe((FragmentActivity) context, aBoolean ->
        {
            if (aBoolean) {
                Toast.makeText(context, context.getText(R.string.deletefromfavsucces), Toast.LENGTH_SHORT).show();
                productsbysubcats.remove(position);
